...
const MyComplaints = () => {
    ...
    const fetchComplaints = async () => {
        setLoading(true);
        try {
            const complaintsRes = await axios.get('http://13.61.64.60:5000/api/complaints/my', {
    ...
    const fetchFeedbacks = async () => {
        try {
            const feedbacksRes = await axios.get('http://13.61.64.60:5000/api/feedback', {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
    ...
    const fetchProfile = async () => {
        try {
            const profileRes = await axios.get('http://13.61.64.60:5000/api/auth/profile', {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
    ...
    const submitFeedback = async (complaintId, rating, comment) => {
        try {
            await axios.post('http://13.61.64.60:5000/api/feedback', { complaintId, rating, comment }, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
            fetchFeedbacks();
        } catch (err) {
