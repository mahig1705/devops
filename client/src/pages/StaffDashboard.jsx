...
const StaffDashboard = () => {
    ...
    const fetchAssignedComplaints = async () => {
        setLoading(true);
        try {
            const res = await axios.get('http://13.61.64.60:5000/api/complaints/assigned', {
    ...
    const fetchProfile = async () => {
        try {
            const res = await axios.get('http://13.61.64.60:5000/api/auth/profile', {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            });
    ...
    const submitStaffUpdate = async (selectedComplaint, formData) => {
        try {
            await axios.post(`http://13.61.64.60:5000/api/complaints/${selectedComplaint._id}/staff-update`, formData, {
                headers: {
